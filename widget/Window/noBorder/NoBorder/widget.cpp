...
{
//    this->setWindowFlags(Qt::Window | Qt::FramelessWindowHint|Qt::WindowMinimizeButtonHint|Qt::WindowMaximizeButtonHint);
    /* 隐藏窗口标题栏 */
    setFlags(flags() | Qt::Window | Qt::FramelessWindowHint|Qt::WindowMinimizeButtonHint|Qt::WindowMaximizeButtonHint);
...
bool Widget::movable() const
{
    return m_movable;
}

void Widget::setMovable(bool arg)
{
    if (m_movable != arg) {
        m_movable = arg;
        emit movableChanged();
...
void Widget::setResizable(bool arg)
{
    if (m_resizable != arg) {
        m_resizable = arg;
...
Widget::MouseArea Widget::getArea(const QPoint &pos)
{
    int x = pos.x();
    int y = pos.y();
    int w = width();
...
void Widget::setWindowGeometry(const QPoint &pos)
{
